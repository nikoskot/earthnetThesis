...
class PatchExpansion(nn.Module):
    ...
    def __init__(self, dim, outputChannels, spatialScale=2, norm=None, kernel=(3,3,3), padding=(1,1,1)):
        super().__init__()
        self.spatialScale = spatialScale
        self.upsampe = nn.Upsample(scale_factor=(1, spatialScale, spatialScale), mode='nearest')
        self.conv = nn.Conv3d(in_channels=dim, out_channels=outputChannels, kernel_size=kernel, stride=(1,1,1), padding=padding, bias=True)
        if norm == 'LayerNorm':
...
class RegressionHead(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.act1 = nn.Hardtanh(min_val=0, max_val=1)
        self.conv2 = nn.Conv3d(self.in_channels // 2, self.in_channels // 2, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1), bias=True)
        self.act2 = nn.Hardtanh(min_val=0, max_val=1)
        self.conv3 = nn.Conv3d(self.in_channels // 2, self.out_channels, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=True)
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        #     self.layers.append(layer)
        self.n = len(config['encoder']['numChannels'])

        if self.n == 3:
            self.patchExpansion1 = PatchExpansion(dim=config['encoder']['numChannels'][-1], outputChannels=config['encoder']['numChannels'][-1] // 2)

            self.conv1 = nn.Conv3d(in_channels=config['encoder']['numChannels'][-1], out_channels=config['encoder']['numChannels'][-1] // 2, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
            self.norm1 = nn.LayerNorm(config['encoder']['numChannels'][-1] // 2)
            self.relu1 = nn.ReLU()
            self.conv2 = nn.Conv3d(in_channels=config['encoder']['numChannels'][-1] // 2, out_channels=config['encoder']['numChannels'][-1] // 2, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
            self.norm2 = nn.LayerNorm(config['encoder']['numChannels'][-1] // 2)
            self.relu2 = nn.ReLU()
            self.patchExpansion2 = PatchExpansion(dim=config['encoder']['numChannels'][-1] // 2, outputChannels=config['encoder']['numChannels'][-1] // 4)

            self.conv3 = nn.Conv3d(in_channels=config['encoder']['numChannels'][-1] // 2, out_channels=config['encoder']['numChannels'][-1] // 4, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
            self.norm3 = nn.LayerNorm(config['encoder']['numChannels'][-1] // 4)
            self.relu3 = nn.ReLU()
            self.conv4 = nn.Conv3d(in_channels=config['encoder']['numChannels'][-1] // 4, out_channels=config['encoder']['numChannels'][-1] // 4, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
            self.norm4 = nn.LayerNorm(config['encoder']['numChannels'][-1] // 4)
            self.relu4 = nn.ReLU()
            self.patchExpansion3 = PatchExpansion(dim=config['encoder']['numChannels'][-1] // 4, outputChannels=config['encoder']['numChannels'][-1] // 8)
    ...
    def forward(self, x_in):
        ...
        if self.n == 3:
            x = self.patchExpansion1(x_in[-1])

            x = torch.cat((x, x_in[-2]), dim=1)
            ...
            x = x.permute(0, 4, 1, 2, 3) # x -> B C T H W
            x = self.relu2(x)
            x = self.patchExpansion2(x)

            x = torch.cat((x, x_in[-3]), dim=1)
...
class VideoSwinUNet(nn.Module):
    def __init__(self, config, logger):
        ...
        self.head = RegressionHead(in_channels=config['C'] // 2, out_channels=config['modelOutputCh'])
...
if __name__ == "__main__":
    ...
    config = load_config('/home/nikoskot/earthnetThesis/experiments/configV3.yml')
