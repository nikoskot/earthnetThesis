...
class BasicLayerEncoder(nn.Module):
    ...
    def forward(self, x, contextWeather, staticData):
        """ Forward function.
        ...
        x = x + F.interpolate(contextWeather, size=(x.shape[2], x.shape[3], x.shape[4])) # Conditioning encoder on context weather data
        x = x + F.interpolate(staticData, size=(x.shape[2], x.shape[3], x.shape[4])) # Conditioning encoder on context weather data
...
class Encoder(nn.Module):
    ...
    def forward(self, x, contextWeather, staticData):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x = layer(x, contextWeather[i], staticData[i])
...
class ContextWeatherEncoder(nn.Module):
    def __init__(self, config):
        super(ContextWeatherEncoder, self).__init__()
        self.conv1 = nn.Conv3d(config['contextWeatherInputCh'], config['C'] // 2, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))  # Reduce size
        self.norm1 = nn.LayerNorm([config['C'] // 2, config['mainInputTime'], config['weatherHeightWidth'], config['weatherHeightWidth']])
        self.conv2 = nn.Conv3d(config['C'] // 2, config['C'], kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
        self.conv3 = nn.Conv3d(config['C'], config['C'] * 2, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
...
class TargetWeatherEncoder(nn.Module):
    def __init__(self, config):
        super(TargetWeatherEncoder, self).__init__()
        self.conv1 = nn.Conv3d(config['targetWeatherInputCh'], config['C'] // 2, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))  # Reduce size
        self.norm1 = nn.LayerNorm([config['C'] // 2, config['outputTime'], config['weatherHeightWidth'], config['weatherHeightWidth']])
        self.conv2 = nn.Conv3d(config['C'] // 2, config['C'], kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
        self.conv3 = nn.Conv3d(config['C'], config['C'] * 2, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
        # self.conv3 = nn.Conv3d(128, config['encoder']['numChannels'][-1], kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
        # self.avgPool3d = nn.AdaptiveAvgPool3d((bottleneck_shape[0], bottleneck_shape[1], bottleneck_shape[2]))
...
class Decoder(nn.Module):
    ...
    def forward(self, x, targetWeather, staticData):
        ...
        for i, x_i in enumerate(reversed(x)):
            ...
            if i != 0:
                ...
                x_out = x_out + F.interpolate(list(reversed(targetWeather))[i], size=(x_out.shape[2], x_out.shape[3], x_out.shape[4])) # Conditioning decoder on target weather data
                x_out = x_out + F.interpolate(list(reversed(staticData))[i], size=(x_out.shape[2], x_out.shape[3], x_out.shape[4])) # Conditioning decoder on static data

                x_i = torch.cat((x_out, x_i), dim=1)

            else:
                x_i = x_i + F.interpolate(list(reversed(targetWeather))[i], size=(x_i.shape[2], x_i.shape[3], x_i.shape[4])) # Conditioning decpder on target weather data
                x_i = x_i + F.interpolate(list(reversed(staticData))[i], size=(x_i.shape[2], x_i.shape[3], x_i.shape[4])) # Conditioning decpder on static data
...
class VideoSwinUNet(nn.Module):
    def __init__(self, config, logger):
        ...
        # self.block1 = BasicLayerEncoder(dim=192, depth=2, num_heads=6, window_size=(3, 7, 7), mlp_ratio=4.0, qkv_bias=False, qk_scale=None, drop=0.0, attn_drop=0.0, drop_path=0.0, norm_layer=None, downsample=PatchMerging, use_checkpoint=False)
        # self.block2 = BasicLayerEncoder(dim=384, depth=2, num_heads=12, window_size=(3, 7, 7), mlp_ratio=4.0, qkv_bias=False, qk_scale=None, drop=0.0, attn_drop=0.0, drop_path=0.0, norm_layer=None, downsample=PatchMerging, use_checkpoint=False)
        self.contextWeatherEncoder = ContextWeatherEncoder(config=config)

        self.targetWeatherEncoder = TargetWeatherEncoder(config=config)
    ...
    def forward(self, contextImg, contextWeather, targetWeather, staticData):

        # Input shape B,C,T,H,W
        x = self.patch_embed(contextImg)
        ...
        x = self.encoder(x, contextWeather, staticData)
        ...
        x = self.decoder(x, targetWeather, staticData)
...
if __name__ == "__main__":
    ...
    config = load_config('/home/nikoskot/earthnetThesis/experiments/configV6.yml')
    ...
    startTime = time.time()
    contextImg = torch.rand(1, 4, 10, 128, 128).to(torch.device('cuda')) # B, C, T, H, W
    ...
    for i in range(20):
        ...
        startTime = time.time()
        y = model(contextImg, contextWeather, targetWeather, staticData)
        endTime = time.time()
